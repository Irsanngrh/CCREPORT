...
export default function DirectorEditPage() {
    const { id } = useParams();
    ...
    useEffect(() => {
        const fetchDirector = async () => {
            try {
                const res = await directorsApi.getOne(id);
                const data = res.data.data;
                setForm({ name: data.name, position: data.position });
                setCreditCards(data.creditCards?.length ? data.creditCards : [{ bankName: '', cardNumber: '' }]);
            } catch (err) {
    ...
    return (
        <Layout>
            <div className="breadcrumb mb-4">
                ...
                <span>Ubah Data Direktur</span>
            </div>

            <div className="page-title-bar">
                <div>
                    <h1 className="page-title">Ubah Data Direktur</h1>
                    <p className="page-subtitle">Pembaruan profil direktur dan daftar kartu kredit terkait</p>
                </div>
            </div>
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            <div className="flex gap-2 mt-6">
                                ...
                                <button type="submit" className="btn btn-primary" disabled={submitting}>
                                    {submitting ? <span className="spinner" style={{ width: 14, height: 14 }} /> : 'Perbarui Perubahan'}
                                </button>
                            </div>
                        </div>
                    </form>
